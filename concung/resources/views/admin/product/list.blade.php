...
<div class="row d-flex justify-content-lg-between mb-3">
  <form method="GET" action="/admin/products/list" class="input-group rounded col-lg-3 w-auto align-items-center">
    <div class="form-outline">
      <input type="search" class="form-control rounded" placeholder="Tìm kiếm" aria-label="Search" aria-describedby="search-addon" name="search" id="search-product" />
    </div>
  ...
  <form role="search" method="get" action="/admin/products/list" class="row col-lg-7">
    ...
    <div class="form-group mb-0 d-flex align-items-center justify-content-around col-lg-5">
      <label class="mb-0">Nhà cung cấp: </label>
      <select name="supplier_id" class="form-control form-select w-50">
        <option value="">Chọn ()</option>
        @foreach ($suppliers as $supplier)
        <option value="{{ $supplier->id }}" {{ $supplier->id == request()->input('supplier_id') ? 'selected' : '' }}>{{ $supplier->name }}</option>
        @endforeach
      </select>
    </div>
